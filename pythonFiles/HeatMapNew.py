...
class HeatMap(object):
    ...
    def genFiles( self, number, system ):
        for index in range(1,number + 1):
            yield system +"_%d%s.%s"%(index, self.subsystem,self.extension)
    ...
    #                  ,wspace=wspace,hspace=hspace)
        ...
        im1 = ax1.imshow(mat2,interpolation='bilinear',extent=[0, axis1, 0, 1000],
               vmax= vmax, vmin= vmin)
        ax1.set_xticks(range(0,axis1,deltax))
        #ax1.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
        ax1.set_xticklabels(list(range(self.initial,self.initial + n1*3,3)))
        ax1.set_yticks(range(0,1000,50))
        ax1.set_yticklabels([str(round(float(label), 2)) for label in np.arange(freqThreshold,0,-deltay)])
