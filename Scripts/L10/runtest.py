...
for name in systemName:
    elandScape=[]
    if baseOpto:
        fileIn=path+"box"+name+"New.dat"
        eorfile=path+name+"L10.eor"
        omdfile=path+name+"L10.omd"
        affineScaled(fileIn,eorfile,omdfile)
    else:
        eorfile=path+name+"L10.omd"
    rCut=getrCut(eorfile)
    lx,ly,lz=getBoxGeometry(eorfile)
    caList=sampleCA(lx,ly,lz,rCut,delta,points)
    scaledfile=path+"Scaled.omd"
    scaledfileStat=path+"Scaled.stat"
    scaledfiledump=scaledfile[:-3]+"dump"
 
    landScape=path+name+".landscape"
